...
func (db *DB) GetStaffDetails(ctx context.Context, userID int) (*Staff, error) {
	var staff Staff
	if err := db.conn.GetContext(ctx, &staff, "SELECT id, wave_id, snowflake, COALESCE(avatar_hash, '') avatar_hash, username, display_name, role FROM staff WHERE id = ?", userID); err != nil {
		return nil, err
	}
